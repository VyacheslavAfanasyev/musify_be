...
export class AuthService {
  ...
  async register(createUserDto: ICreateUserDto) {
    try {
      ...
      // 2. Отправляем событие в User Service для создания профиля
      try {
        await this.userClient.emit("user.created", {
          userId: savedUser.id,
          username: createUserDto.username,
          role: createUserDto.role || "listener",
        });

        // 3. Проверяем через 5 секунд, создался ли профиль
        setTimeout(async () => {
          const profileExists = await this.checkProfileExists(savedUser.id);
          if (!profileExists) {
            // Откатываем изменения - удаляем пользователя из PostgreSQL
            await this.authUserRepository.delete(savedUser.id);
            console.error(
              `Failed to create profile for user ${savedUser.id}, rolled back`,
            );
          }
        }, 5000);
      } catch (error) {
        // Если не удалось отправить событие, откатываем
        await this.authUserRepository.delete(savedUser.id);
        return {
          success: false,
